...
class _OnBoardingState extends State<OnBoarding> {
  ...
  Future<void> _apigetUserLocation() async {
    try {
      ...
      if (userLocationModel?.status == "true") {
        await _apiBoarding();
      }
    } catch (e) {
      log(e.toString());
  ...
  Future<void> _apiBoarding() async {
    try {
      ...
      Map _body = {
        "countryid": userLocationModel?.predResult?.countryId,
        "stateid": userLocationModel?.predResult?.stateId,
        "districtid": userLocationModel?.predResult?.districtId,
        "cityid": userLocationModel?.predResult?.cityId,
      };
      ...
      if (mounted) {
        setState(() {
          ...
          SharedPreference.setValue(
              PrefConstants.LATITUDE, latitude1.toString());
          SharedPreference.setValue(
              PrefConstants.LONGITUDE, longitude1.toString());
